...
//    alert(routeUrl)
    ...
    $form_login.find('input[type="submit"]').on('click', function (event) {
        ...
        var signin_url = $("#signin_url").val()
        
        if (email == '') {
            $('#loginErrMsg').html('This field is required');
            $('#pwdErrMsg').html('');
            return false;
        }
        if (password == '') {
            $('#loginErrMsg').html('');
            $('#pwdErrMsg').html('This field is required');
            return false;
        }
    ...
    $('#signupBtn').on('click', function () {
        ...
        var password_confirmation = $("#signup-cpassword").val()
        var dob = $("#signup-dob").val()
        var role_type = $("#signupRole").val()
        var user_type = $("#signupUserType").val()
        var signup_url = $("#signup_url").val()

        if (password != password_confirmation) {
            $('#signUpErr').html('The password confirmation does not match.');
        }
        if (email != '') {
            $('#signUpErr').html('Please fill all filled');
        }
...
//                $(btn).attr('disabled', true);
            ...
            success: function (response) {
                if (response.resCode == 0) {
					...
					$('#signuperr').text(response.resMsg);
//                    location.reload();
                } else {
                    $('#server_resposne_msg').html(response.resMsg);
					$('#signuperr').text(response.resMsg);
