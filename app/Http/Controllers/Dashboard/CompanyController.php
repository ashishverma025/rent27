    ...
    public function fetchCompany(Request $request) {
        ...
        if (count($company) > 0) {
            foreach ($company as $comp) {
                $compImage = !empty($comp->company_logo) ? asset('storage/uploads/company') . '/' . $comp->company_logo : asset('storage/images') . '/not-available.jpg';
                ...
                $response['aaData'][$k] = [$k + 1, "<img src='$compImage' height='50' width='50'>",$comp->company_name, $action];
                $k++;
            }
        }
...
//                        prd($CompanyDetails);
        ...
        if ($request->isMethod('post')) {
            ...
            if (!empty($companyLogo)) {
                $logoFileName = upload_admin_images($companyLogo, 'company');
