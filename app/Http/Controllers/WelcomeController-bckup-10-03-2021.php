...
//        pr($truckList);
    ...
    public function truckForSale(Request $request) {
        ...
        // Redirect to Dealer vehical page (Dashboard)
        $this->redirectDealerVehical();
...
//        pr($truckList);
    ...
    public function redirectDealerVehical()
    {
        if (Auth::check()) {
            $userData = Auth::user();
...
//        pr($sessionData);
    ...
    public function subscription(Request $request) {
        $sessionData = $request->session()->get('quotesData');
//        pr($sessionData);
     $users = DB::select('select * from plans where id > 2');
	       return view('sites.subscription',['users'=>$users]);
        //return view('sites.subscription');
    }
