      ...
      return actions.order.capture().then(function(details) {
        ...
        if(details.status === "COMPLETED")
        {
          ...
          $.ajax({
            ...
            url: '{{url("/handlePaymentResponse")}}',
            type: 'POST',
            /* send the csrf-token and the input to the controller */
            data: {_token: CSRF_TOKEN, payment_response: payment_response, transaction_id: transaction_id, status: status, amount: amount, currency_code: currency_code},
            dataType: 'JSON',
            /* remind that 'data' is the response of the AjaxController */
            success: function (data) {
              window.alert('Thank you for your purchase!');
              window.location.replace('{{url("/handlePaymentResponse")}}');
            }
          });
        }
      });
